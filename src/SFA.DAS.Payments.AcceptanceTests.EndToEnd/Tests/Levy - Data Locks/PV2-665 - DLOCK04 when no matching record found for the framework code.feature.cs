// ------------------------------------------------------------------------------
//  <auto-generated>
//      This code was generated by SpecFlow (http://www.specflow.org/).
//      SpecFlow Version:2.4.0.0
//      SpecFlow Generator Version:2.4.0.0
// 
//      Changes to this file may cause incorrect behavior and will be lost if
//      the code is regenerated.
//  </auto-generated>
// ------------------------------------------------------------------------------
#region Designer generated code
#pragma warning disable
namespace SFA.DAS.Payments.AcceptanceTests.EndToEnd.Tests.Levy_DataLocks
{
    using TechTalk.SpecFlow;
    
    
    [System.CodeDom.Compiler.GeneratedCodeAttribute("TechTalk.SpecFlow", "2.4.0.0")]
    [System.Runtime.CompilerServices.CompilerGeneratedAttribute()]
    [NUnit.Framework.TestFixtureAttribute()]
    [NUnit.Framework.DescriptionAttribute("DLOCK04 - When no matching record found in an employer digital account for for th" +
        "e framework code then datalock DLOCK_04 will be produced PV2-665")]
    public partial class DLOCK04_WhenNoMatchingRecordFoundInAnEmployerDigitalAccountForForTheFrameworkCodeThenDatalockDLOCK_04WillBeProducedPV2_665Feature
    {
        
        private TechTalk.SpecFlow.ITestRunner testRunner;
        
#line 1 "PV2-665 - DLOCK04 when no matching record found for the framework code.feature"
#line hidden
        
        [NUnit.Framework.OneTimeSetUpAttribute()]
        public virtual void FeatureSetup()
        {
            testRunner = TechTalk.SpecFlow.TestRunnerManager.GetTestRunner();
            TechTalk.SpecFlow.FeatureInfo featureInfo = new TechTalk.SpecFlow.FeatureInfo(new System.Globalization.CultureInfo("en-US"), "DLOCK04 - When no matching record found in an employer digital account for for th" +
                    "e framework code then datalock DLOCK_04 will be produced PV2-665", "\tAs a Provider,\r\n\tI want to be notified with a DLOCK04 when no matching record fo" +
                    "und in an employer digital account for the Framework Code\r\n\tSo that I can correc" +
                    "t the data mis-match between the Commitment and ILR ", ProgrammingLanguage.CSharp, ((string[])(null)));
            testRunner.OnFeatureStart(featureInfo);
        }
        
        [NUnit.Framework.OneTimeTearDownAttribute()]
        public virtual void FeatureTearDown()
        {
            testRunner.OnFeatureEnd();
            testRunner = null;
        }
        
        [NUnit.Framework.SetUpAttribute()]
        public virtual void TestInitialize()
        {
        }
        
        [NUnit.Framework.TearDownAttribute()]
        public virtual void ScenarioTearDown()
        {
            testRunner.OnScenarioEnd();
        }
        
        public virtual void ScenarioInitialize(TechTalk.SpecFlow.ScenarioInfo scenarioInfo)
        {
            testRunner.OnScenarioInitialize(scenarioInfo);
            testRunner.ScenarioContext.ScenarioContainer.RegisterInstanceAs<NUnit.Framework.TestContext>(NUnit.Framework.TestContext.CurrentContext);
        }
        
        public virtual void ScenarioStart()
        {
            testRunner.OnScenarioStart();
        }
        
        public virtual void ScenarioCleanup()
        {
            testRunner.CollectScenarioErrors();
        }
        
        [NUnit.Framework.TestAttribute()]
        [NUnit.Framework.DescriptionAttribute("DLOCK04 - When no matching record found in an employer digital account for for th" +
            "e framework code then datalock DLOCK_04 will be produced PV2-665")]
        [NUnit.Framework.TestCaseAttribute("R01/Current Academic Year", null)]
        [NUnit.Framework.TestCaseAttribute("R02/Current Academic Year", null)]
        [NUnit.Framework.TestCaseAttribute("R03/Current Academic Year", null)]
        [NUnit.Framework.TestCaseAttribute("R04/Current Academic Year", null)]
        [NUnit.Framework.TestCaseAttribute("R05/Current Academic Year", null)]
        [NUnit.Framework.TestCaseAttribute("R06/Current Academic Year", null)]
        [NUnit.Framework.TestCaseAttribute("R07/Current Academic Year", null)]
        [NUnit.Framework.TestCaseAttribute("R08/Current Academic Year", null)]
        [NUnit.Framework.TestCaseAttribute("R09/Current Academic Year", null)]
        [NUnit.Framework.TestCaseAttribute("R10/Current Academic Year", null)]
        [NUnit.Framework.TestCaseAttribute("R11/Current Academic Year", null)]
        [NUnit.Framework.TestCaseAttribute("R12/Current Academic Year", null)]
        public virtual void DLOCK04_WhenNoMatchingRecordFoundInAnEmployerDigitalAccountForForTheFrameworkCodeThenDatalockDLOCK_04WillBeProducedPV2_665(string collection_Period, string[] exampleTags)
        {
            TechTalk.SpecFlow.ScenarioInfo scenarioInfo = new TechTalk.SpecFlow.ScenarioInfo("DLOCK04 - When no matching record found in an employer digital account for for th" +
                    "e framework code then datalock DLOCK_04 will be produced PV2-665", null, exampleTags);
#line 7
this.ScenarioInitialize(scenarioInfo);
            this.ScenarioStart();
#line 9
 testRunner.Given("the employer levy account balance in collection period R12/Current Academic Year " +
                    "is 11000", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Given ");
#line hidden
            TechTalk.SpecFlow.Table table1371 = new TechTalk.SpecFlow.Table(new string[] {
                        "Identifier",
                        "framework code",
                        "programme type",
                        "pathway code",
                        "agreed price",
                        "start date",
                        "end date",
                        "status",
                        "effective from"});
            table1371.AddRow(new string[] {
                        "Apprenticeship a",
                        "593",
                        "20",
                        "1",
                        "10000",
                        "01/Aug/CurrentAcademic Year",
                        "01/Aug/Next Academic Year",
                        "active",
                        "01/Aug/CurrentAcademic Year"});
#line 10
 testRunner.And("the following commitments exist", ((string)(null)), table1371, "And ");
#line hidden
            TechTalk.SpecFlow.Table table1372 = new TechTalk.SpecFlow.Table(new string[] {
                        "Start Date",
                        "Planned Duration",
                        "Total Training Price",
                        "Total Training Price Effective Date",
                        "Completion Status",
                        "Contract Type",
                        "Aim Sequence Number",
                        "Aim Reference",
                        "Framework code",
                        "Programme type",
                        "Pathway code",
                        "Funding Line Type",
                        "SFA Contribution Percentage"});
            table1372.AddRow(new string[] {
                        "01/Aug/CurrentAcademic Year",
                        "12 months",
                        "10000",
                        "01/Aug/CurrentAcademic Year",
                        "continuing",
                        "Act1",
                        "1",
                        "ZPROG001",
                        "589",
                        "20",
                        "1",
                        "16-18 Apprenticeship (From May 2017) Levy Contract",
                        "90%"});
#line 13
 testRunner.And("the provider is providing training for the following learners", ((string)(null)), table1372, "And ");
#line hidden
            TechTalk.SpecFlow.Table table1373 = new TechTalk.SpecFlow.Table(new string[] {
                        "Price Episode Id",
                        "Total Training Price",
                        "Total Training Price Effective Date",
                        "Contract Type",
                        "SFA Contribution Percentage"});
            table1373.AddRow(new string[] {
                        "pe-1",
                        "10000",
                        "01/Aug/CurrentAcademic Year",
                        "Act1",
                        "90%"});
#line 16
    testRunner.And("price details as follows", ((string)(null)), table1373, "And ");
#line 19
 testRunner.When(string.Format("the ILR file is submitted for the learners for collection period {0}", collection_Period), ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "When ");
#line hidden
            TechTalk.SpecFlow.Table table1374 = new TechTalk.SpecFlow.Table(new string[] {
                        "Delivery Period",
                        "On-Programme",
                        "Completion",
                        "Balancing",
                        "First16To18EmployerIncentive",
                        "First16To18ProviderIncentive",
                        "Second16To18EmployerIncentive",
                        "Second16To18ProviderIncentive",
                        "OnProgramme16To18FrameworkUplift",
                        "LearningSupport",
                        "FirstDisadvantagePayment",
                        "SecondDisadvantagePayment",
                        "Price Episode Identifier"});
            table1374.AddRow(new string[] {
                        "Aug/Current Academic Year",
                        "666.66667",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "120",
                        "150",
                        "0",
                        "0",
                        "pe-1"});
            table1374.AddRow(new string[] {
                        "Sep/Current Academic Year",
                        "666.66667",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "120",
                        "150",
                        "0",
                        "0",
                        "pe-1"});
            table1374.AddRow(new string[] {
                        "Oct/Current Academic Year",
                        "666.66667",
                        "0",
                        "0",
                        "500",
                        "500",
                        "0",
                        "0",
                        "120",
                        "150",
                        "0",
                        "0",
                        "pe-1"});
            table1374.AddRow(new string[] {
                        "Nov/Current Academic Year",
                        "666.66667",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "120",
                        "150",
                        "300",
                        "0",
                        "pe-1"});
            table1374.AddRow(new string[] {
                        "Dec/Current Academic Year",
                        "666.66667",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "120",
                        "150",
                        "0",
                        "0",
                        "pe-1"});
            table1374.AddRow(new string[] {
                        "Jan/Current Academic Year",
                        "666.66667",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "120",
                        "150",
                        "0",
                        "0",
                        "pe-1"});
            table1374.AddRow(new string[] {
                        "Feb/Current Academic Year",
                        "666.66667",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "120",
                        "150",
                        "0",
                        "0",
                        "pe-1"});
            table1374.AddRow(new string[] {
                        "Mar/Current Academic Year",
                        "666.66667",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "120",
                        "150",
                        "0",
                        "0",
                        "pe-1"});
            table1374.AddRow(new string[] {
                        "Apr/Current Academic Year",
                        "666.66667",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "120",
                        "150",
                        "0",
                        "0",
                        "pe-1"});
            table1374.AddRow(new string[] {
                        "May/Current Academic Year",
                        "666.66667",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "120",
                        "150",
                        "0",
                        "0",
                        "pe-1"});
            table1374.AddRow(new string[] {
                        "Jun/Current Academic Year",
                        "666.66667",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "0",
                        "120",
                        "150",
                        "0",
                        "0",
                        "pe-1"});
            table1374.AddRow(new string[] {
                        "Jul/Current Academic Year",
                        "666.66667",
                        "0",
                        "0",
                        "0",
                        "0",
                        "500",
                        "500",
                        "120",
                        "150",
                        "0",
                        "300",
                        "pe-1"});
#line 20
 testRunner.Then("the following learner earnings should be generated", ((string)(null)), table1374, "Then ");
#line hidden
            TechTalk.SpecFlow.Table table1375 = new TechTalk.SpecFlow.Table(new string[] {
                        "Apprenticeship",
                        "Delivery Period",
                        "Framework Code",
                        "Programme Type",
                        "Pathway Code",
                        "Transaction Type",
                        "Error Code",
                        "Price Episode Identifier"});
            table1375.AddRow(new string[] {
                        "Apprenticeship a",
                        "Aug/Current Academic Year",
                        "589",
                        "20",
                        "1",
                        "Learning",
                        "DLOCK_04",
                        "pe-1"});
            table1375.AddRow(new string[] {
                        "Apprenticeship a",
                        "Sep/Current Academic Year",
                        "589",
                        "20",
                        "1",
                        "Learning",
                        "DLOCK_04",
                        "pe-1"});
            table1375.AddRow(new string[] {
                        "Apprenticeship a",
                        "Oct/Current Academic Year",
                        "589",
                        "20",
                        "1",
                        "Learning",
                        "DLOCK_04",
                        "pe-1"});
            table1375.AddRow(new string[] {
                        "Apprenticeship a",
                        "Nov/Current Academic Year",
                        "589",
                        "20",
                        "1",
                        "Learning",
                        "DLOCK_04",
                        "pe-1"});
            table1375.AddRow(new string[] {
                        "Apprenticeship a",
                        "Dec/Current Academic Year",
                        "589",
                        "20",
                        "1",
                        "Learning",
                        "DLOCK_04",
                        "pe-1"});
            table1375.AddRow(new string[] {
                        "Apprenticeship a",
                        "Jan/Current Academic Year",
                        "589",
                        "20",
                        "1",
                        "Learning",
                        "DLOCK_04",
                        "pe-1"});
            table1375.AddRow(new string[] {
                        "Apprenticeship a",
                        "Feb/Current Academic Year",
                        "589",
                        "20",
                        "1",
                        "Learning",
                        "DLOCK_04",
                        "pe-1"});
            table1375.AddRow(new string[] {
                        "Apprenticeship a",
                        "Mar/Current Academic Year",
                        "589",
                        "20",
                        "1",
                        "Learning",
                        "DLOCK_04",
                        "pe-1"});
            table1375.AddRow(new string[] {
                        "Apprenticeship a",
                        "Apr/Current Academic Year",
                        "589",
                        "20",
                        "1",
                        "Learning",
                        "DLOCK_04",
                        "pe-1"});
            table1375.AddRow(new string[] {
                        "Apprenticeship a",
                        "May/Current Academic Year",
                        "589",
                        "20",
                        "1",
                        "Learning",
                        "DLOCK_04",
                        "pe-1"});
            table1375.AddRow(new string[] {
                        "Apprenticeship a",
                        "Jun/Current Academic Year",
                        "589",
                        "20",
                        "1",
                        "Learning",
                        "DLOCK_04",
                        "pe-1"});
            table1375.AddRow(new string[] {
                        "Apprenticeship a",
                        "Jul/Current Academic Year",
                        "589",
                        "20",
                        "1",
                        "Learning",
                        "DLOCK_04",
                        "pe-1"});
#line 34
 testRunner.And("the following data lock failures were generated", ((string)(null)), table1375, "And ");
#line 48
 testRunner.And("Month end is triggered", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "And ");
#line 49
 testRunner.And("no provider payments will be generated", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "And ");
#line 50
 testRunner.And("no provider payments will be recorded", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "And ");
#line hidden
            this.ScenarioCleanup();
        }
    }
}
#pragma warning restore
#endregion
